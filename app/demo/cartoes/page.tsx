...
export default function DemoCartoes() {
  ...
  const abrirNovo = () => {
    setEditando(null);
    setForm({ nome: '', cor: '#494bd6', limite: '', fechamento: '5', vencimento: '10' });
    setShowModal(true);
  };

  const abrirEditar = (c: any) => {
    setEditando(c);
    setForm({ nome: c.nome, cor: c.cor, limite: String(c.limite || ''), fechamento: String(c.fechamento), vencimento: String(c.vencimento) });
    setShowModal(true);
  };

  const salvar = () => {
    const payload = { nome: form.nome, cor: form.cor, limite: parseFloat(form.limite) || 0, fechamento: parseInt(form.fechamento), vencimento: parseInt(form.vencimento), ativo: true };
    if (editando) demo.updateCartao(editando.id, payload);
    else demo.addCartao(payload);
    setShowModal(false);
  };

  const excluir = (id: number) => {
    demo.deleteCartao(id);
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div style={{ color: 'var(--outline)', fontSize: '13px', marginTop: '4px' }}>{demo.cartoes.length} cartões ativos</div>
        </div>
        <Button variant="primary" onClick={abrirNovo}>+ Novo cartão</Button>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '16px' }}>
        {demo.cartoes.map((c: any) => (
          <div key={c.id} style={{ background: `linear-gradient(135deg, ${c.cor}f0 0%, ${c.cor}b0 60%, ${c.cor}88 100%)`, borderRadius: '20px', padding: '24px', position: 'relative', overflow: 'hidden', boxShadow: `0 12px 40px ${c.cor}55, 0 4px 12px rgba(0,0,0,0.25)`, minHeight: '160px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.72)', marginTop: '5px', fontFamily: 'JetBrains Mono, monospace', display: 'flex', gap: '8px' }}>
                  <span>fecha {c.fechamento}</span><span>·</span><span>vence {c.vencimento}</span>
                </div>
              </div>
            ...
            {Number(c.limite) > 0 && (
              <div style={{ margin: '16px 0 0' }}>
                ...
                <div style={{ height: '4px', background: 'rgba(255,255,255,0.22)', borderRadius: '99px', overflow: 'hidden' }}>
                  <div style={{ height: '100%', borderRadius: '99px', width: '0%', background: 'rgba(255,255,255,0.80)' }} />
                </div>
              </div>
            )}
            <div style={{ display: 'flex', gap: '8px', marginTop: '20px' }}>
              <button onClick={() => abrirEditar(c)} style={{ flex: 1, background: 'rgba(255,255,255,0.18)', border: '1px solid rgba(255,255,255,0.30)', color: '#fff', borderRadius: '10px', padding: '9px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>✎ Editar</button>
              <button onClick={() => setConfirmandoExcluir(c.id)} style={{ background: 'rgba(239,68,68,0.22)', border: '1px solid rgba(239,68,68,0.35)', color: 'rgba(255,200,200,0.95)', borderRadius: '10px', padding: '9px 16px', fontSize: '13px', cursor: 'pointer' }}>✕</button>
            </div>
          </div>
        ))}
      ...
      {confirmandoExcluir !== null && (
        <ConfirmarModal
          ...
          detalhe="O cartão será removido do modo demo."
        ...
        <ModalBase title={editando ? 'Editar cartão' : 'Novo cartão'} onClose={() => setShowModal(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div>
              ...
              <input value={form.nome} onChange={e => setForm({ ...form, nome: e.target.value })} placeholder="Ex: Cartão Nubank" />
            </div>
            <div>
            ...
            <div>
              <label style={{ fontSize: '12px', color: 'var(--outline)', display: 'block', marginBottom: '6px', fontFamily: 'JetBrains Mono, monospace', letterSpacing: '0.05em' }}>LIMITE (R$) — OPCIONAL</label>
              <input type="number" step="100" value={form.limite} onChange={e => setForm({ ...form, limite: e.target.value })} placeholder="0" />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <div>
                <label style={{ fontSize: '12px', color: 'var(--outline)', display: 'block', marginBottom: '6px', fontFamily: 'JetBrains Mono, monospace', letterSpacing: '0.05em' }}>FECHA DIA</label>
                <input type="number" min="1" max="31" value={form.fechamento} onChange={e => setForm({ ...form, fechamento: e.target.value })} />
              </div>
              <div>
                <label style={{ fontSize: '12px', color: 'var(--outline)', display: 'block', marginBottom: '6px', fontFamily: 'JetBrains Mono, monospace', letterSpacing: '0.05em' }}>VENCE DIA</label>
